...
class Model():
	...
	def optimize(self, kind, *, objective, direction = 'max', flux_bounds = (0, 100), conc_bounds = None, 
				 preset_fluxes = None, preset_concs = None, irr_reactions = None, excluded_concs = None, 
				 excluded_mb = None, excluded_thmd = None, included_epc = None, use_fba_results = False,
		...
		direction: str
			...
			enzyme_protein_cost/objective_flux or enzyme_protein_cost which will always be optimized in 
			minimizing direction
		
		flux_bounds: tuple
			lower and upper bound of metabolic fluxes in mmol/gCDW/h, valid in 'fba', 'tfba' and 'etfba'
			
		conc_bounds: tuple
			lower and upper bound of metabolite concentrations in mM, valid in 'tfba' and 'etfba'
			
		preset_fluxes: dict
			rxnid => float, fixed metabolic fluxes, e.g., substrate update rate, preset values override 
			reversibilities set by irr_reactions, valid in 'fba', 'tfba' and 'etfba'
		...
		irr_reactions: list of reaction ID
			...
			overwrite those defined in file. valid in 'fba', 'tfba' and 'etfba'
		...
		included_epc: list of reaction ID
			...
			if kinetic parameters of Km, kcat and M.W. are not available for included reactions, default 
			values will be used
		...
		if kind.lower() == 'etfba':
			...
			if (use_fba_results and use_tfba_results) or (not use_fba_results and not use_tfba_results):
				raise ValueError('either use_fba_results or use_tfba_results should be True')
