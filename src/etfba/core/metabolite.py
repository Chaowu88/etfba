...
#         cls._init[cls] = attrs.get('__init__', None)
...
#         init = cls._init[cls]
#         if init is not None:
#             key = sum(hash(argv) for argv in args) + sum(hash(argv) for argv in kwargs.values())
#         else:
#             key = cls
...
class Metabolite(metaclass = Singleton):
    '''
    Metabolite instances created with identical IDs, names, and all 
    other attributes are treated as a singular instance.

    Attributes
    ----------
    kms: PrettyDict
        Key is the reaction ID where the metabolite is involved, 
        value is the corresponding Km value.
    ...
    coes: PrettyDict
        Key is the reaction ID where the metabolite is involved, 
        value is the corresponding stoichiometric coefficient in the 
        reaction. Negative for substrates and positive for products.
    
    coe: positive float
        Stoichiometric coefficient of the metabolite in the host 
        reaction.
    ...
    def __init__(
        ...
        name: str
            Metabolite name.
        compartment: str
            Compartment identifier: "c" for cytoplasm; "p" for 
            periplasm; "e" for extracellular.
        is_h: bool
    ...
    # @property
    # def is_exch_metabolite(self):
        
    #     return bool(re.match(r'^[\w\._]+\.o$', self.metabid))


    # @property
    # def is_biomass(self):

    #     return bool(re.match(r'^biomass$', self.metabid, flags = re.I))    
